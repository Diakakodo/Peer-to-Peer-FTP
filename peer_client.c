...
int exec_query_client(int sock, struct sockaddr_in server_addr, char *connection_type) {
	...
	if(strcmp(arguments[0],"Ping") == 0) {
		...
		pout("\tftp commands (Case Sensitive):\n");
...
void serverside_file_download_func(int sock, int connection, struct sockaddr_in client_addr, char* server_share_dir,char *connection_type) {
	...
	if(fp == NULL) {
		if (strcmp(connection_type, "tcp") == 0) {
			send (connection, "File does not exist&&&****@@@\0",1024,0);
		}
		else {
			sendto(sock, "File does not exist&&&****@@@\0",1024, 0,(struct sockaddr *)&client_addr, sizeof(struct sockaddr));
		}
	}
	else {
		if (strcmp(connection_type, "tcp") == 0) {
...
int peer_server(char *server_share_dir, int server_port_number, char *connection_type, int server_upload_flag) {
	...
	while(1) {
		...
		if (strcmp(connection_type, "tcp") == 0) {
			...
			fprintf(stdout,"[SERVER] Connection established with %s - %d\n", 
		...
		while(1) {
			...
			server_recv_data[bytes_recv] = '\0';
			fprintf(stdout,"[SERVER] Request Recieved - \"%s\"\n", server_recv_data);
			parse_request();
...
void init_help() {
	pout("Incorrect number of Arguments given to ftp.\n");
	pout("USAGE: ./ftp protocol --server share_dir upload_flag server_port --client listening_ip listening_port\n");
	...
	pout("For Example :\n./ftp tcp --server . 1 9000 --client localhost 9001\n");
	pout("./ftp tcp --server . 1 9001 --client localhost 9000\n");
}
